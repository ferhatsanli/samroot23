...
Nearest dynamic-to-disabled comparison identifies the implementation: FZDP B9 replaces CYB4’s 71-instruction evaluator (policy `+0xF0`, runtime `+0xBC0/+0xBC4`, policy `+0x154/+0x15C`) with a 28-instruction helper that retains policy logging then returns false unconditionally. The two normal callers and downstream long-press/confirmation/transition bodies remain structurally present. CYB4-only OEM/FRP diagnostics (`[FRP][OEM] init succeed!`, OEM unlock value, PLC) are absent in FZDP. This is direct code-policy evidence, not a demonstrated KG/FRP/VaultKeeper causal link or a consumer EM-token workflow.
